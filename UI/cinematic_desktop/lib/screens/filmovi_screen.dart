...
class _FilmoviScreenState extends State<FilmoviScreen> {
  ...
  late ProjekcijaProvider _projekcijaProvider;
  List<Film> _filmovi = [];
  ...
  void initState() {
    ...
    _projekcijaProvider = context.read<ProjekcijaProvider>();
    _loadData();
  }
  ...
  Future<void> _deleteFilm(Film film) async {
    final projekcije = await _projekcijaProvider.getByFilm(film.id!);
    final imaAktivnih = projekcije.isNotEmpty;
    if (imaAktivnih) {
      await showDialog(
        context: context,
        builder: (context) => AlertDialog(
          title: Text("Brisanje nije dozvoljeno"),
          content: Text(
            "Film ima aktivne projekcije.\n"
            "Ne možete obrisati film dok sve projekcije ne prođu ili dok ih ne sakrijete.",
          ),
          actions: [
            TextButton(
